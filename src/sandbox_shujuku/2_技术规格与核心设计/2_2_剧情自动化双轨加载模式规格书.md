...
## 三、 模式二：基于静态关系 DAG 与滑动视窗的单点 Hook 注入模式 (Sliding Node-Window Hook)

这是面向未来、Token 消耗极低、最智能的极致省流细粒度加载模式。它无须对世界书文本本身做任何破坏性的重写，而是将“一幕”在内存中按节点检索，并通过唯一的酒馆单点宏完成动态手术式注入。
...
sequenceDiagram
    ...
    participant PreAgent as 侦察兵 (前置小模型)
    participant SQLite as SQLite内存库 (plot_nodes)
    ...
    Note over Interceptor, SQLite: 零Agent参与: 纯代码读取本地DAG与正则索引查询相邻节点
    Interceptor->>SQLite: 查询 [RI5] 及其相邻节点数据
    SQLite-->>Interceptor: 返回 RI3(概要), RI4(概要), RI5(全量+对话参考), RI6(概要), RI7(概要)
...
### 3.1 核心机制
* **标准 DAG 关系与本地静态解析**：
  * **静态 DAG 索引定义**：在导入的剧情世界书 JSON 中，增加一个专门的标准 JSON 格式条目，由剧本作者事先白纸黑字定义剧情节点的先后、并行、分支、依赖等关系（**这完全是一个静态指路牌，不需要大模型参与编排**）。
  * **静态解析器 (Local Plot Parser)**：插件在冷启动或换卡时，直接通过纯 TS 脚本和正则表达式扫描导入的剧情世界书文件。在 **50ms 内**，快速、确定性地将各个剧情节点的剧本（Full Script）、概要（Summary）、重要对话参考（Dialogue References）以及对应的 DAG 节点依赖提取出来，灌入内存 SQLite 的 `plot_nodes` 缓存表中建立高效检索索引。
* **滑动节点视窗 (Sliding Node Window)**：
  前置侦察兵判定当前处于节点 $N$（如 `RI5`）。系统直接向 SQLite 查询，无视酒馆的原有条件限制，在内存中瞬间计算并捞出相邻的滑动视窗节点：
