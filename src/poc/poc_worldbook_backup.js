...
(function () {
  ...
  btnBackup.addEventListener('click', async () => {
    ...
    try {
      ...
      const isCreated = await window.TavernHelper.createOrReplaceWorldbook(backupWbName, originalEntries, {
        render: 'debounced',
      });
      ...
      if (backupEntries.length !== originalEntries.length) {
        log(
          `<span style="color:red">❌ 警告：备份条目数不匹配! 原:${originalEntries.length}, 备份:${backupEntries.length}</span>`,
        );
      } else {
