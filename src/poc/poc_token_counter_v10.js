  ...
  eventOnce(tavern_events.CHAT_COMPLETION_PROMPT_READY, async data => {
    ...
    try {
      ...
      if (typeof SillyTavern !== 'undefined' && typeof SillyTavern.getTokenCountAsync === 'function') {
        ...
        console.log(
          `%c[Token Counter POC v10] Token计算完成: ${tokensFound}`,
  ...
  try {
    ...
    const context =
      typeof SillyTavern !== 'undefined' && typeof SillyTavern.getContext === 'function'
        ...
        : typeof window !== 'undefined' && window.SillyTavern
...
$(() => {
  const btn = $(
    '<button style="position:fixed;bottom:50px;left:10px;z-index:9999;padding:10px;background:#5bc0de;color:white;border:none;border-radius:4px;cursor:pointer;">执行 v10 测试</button>',
  );
  btn.on('click', main);
  $('body').append(btn);
