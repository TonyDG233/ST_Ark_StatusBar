 ...
 * 
 * 修改为纯 JS 可以在控制台直接运行测试。
 * 测试目标：
 * 验证通过事件截获的回传数据
 * 旨在直接通过酒馆上下文进行一次 DryRun 触发世界书扫描，并比对返回的条目状态 (disable) 与本地状态 (enabled)。
 */
...
async function testMatchLocalState() {
    ...
    const tavernEventEnum = typeof tavern_events !== 'undefined' ? tavern_events : (window.parent?.tavern_events || window.tavern_events);

    safeEventOnce(tavernEventEnum.WORLD_INFO_ACTIVATED, (entries) => {
        console.log("%c[Match POC] 捕获到 WORLD_INFO_ACTIVATED", "color: blue; font-weight: bold;");
        
        let mismatchCount = 0;
        
        // 4. 遍历返回的条目并与本地比对
        entries.forEach((returnedEntry) => {
            // 如果返回的条目不属于我们绑定的当前主要世界书，跳过测试
            if (returnedEntry.world !== targetWorldbook) {
                console.log(`[Match POC] 跳过外部世界书条目: [${returnedEntry.world}] ${returnedEntry.comment || returnedEntry.uid}`);
                return;
            }

            // 在本地世界书中寻找对应 UID 的条目
            const localEntry = localEntries.find((e) => e.uid === returnedEntry.uid);
            ...
            if (matchStatus) {
                 console.log(`%c[Match POC] 状态吻合 - UID [${returnedEntry.uid}] [${returnedEntry.comment}] Local_Enabled: ${isLocalEnabled}, Returned_Disable: ${returnedEntry.disable}`, 'color: green;');
            } else {
                 console.log(`%c[Match POC] 状态冲突 - UID [${returnedEntry.uid}] [${returnedEntry.comment}] Local_Enabled: ${isLocalEnabled}, Returned_Disable: ${returnedEntry.disable}`, 'color: red;');
                 mismatchCount++;
            }
        });

        if (mismatchCount === 0) {
            console.log("%c[Match POC] 结论: 回传数据的 disable 状态与本地 enabled 状态完全一致！", 'font-size: 14px; font-weight:bold; color: green;');
        } else {
            console.log(`%c[Match POC] 结论: 发现 ${mismatchCount} 处状态不一致！必须以本地状态为准进行校验。`, 'font-size: 14px; font-weight:bold; color: red;');
        }
    });

    // 发起一次 DryRun
    try {
        const st = typeof window.SillyTavern !== 'undefined' ? window.SillyTavern : window.parent?.SillyTavern;
...
$(() => {
    const btn = $('<button style="position:fixed;top:110px;left:10px;z-index:9999;padding:10px;background:lightblue;color:black;">测试数据吻合度</button>');
