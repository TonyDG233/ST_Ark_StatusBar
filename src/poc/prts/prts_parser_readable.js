...
let mc = match[5].matchAll(scenario.regex.animatepara);
...
let [sx,sy,px,py] = exFun.charPos(n,temp.ps);
					if(temp.o3.attr(\"data-n\") == n){
						...
						if(cmd_set.fadetime) temp.o3.hide().fadeIn(cmd_set.fadetime*950);/* 杩欓噷灏辨槸闇€瑕佸垽瀹歠adetime */
					}
					else{
...
let py = Math.max((556 - op_len * 80) / 2,72);/* 556=588-32@鍚戜笂鍋忕Щ32px */
...
let p = document.createElement(\"span\");
								pb.append(p);
								p.innerHTML = \"銆怽" + ele.innerHTML + \"銆慭";
...
case "largeimg":
				...
				if(temp.ig.length == 0 || temp.sw.length == 0 || temp.sh.length == 0) 
return -1;
...
let px = (base_width-sx)/2,py = (base_height-sy)/2;
					...
					for(
let i = 0,offx = 0,offy = 0;i<leng;i++){
...
let px = (base_width-sx)/2,py = (base_height-sy)/2;
					temp.o2.attr({width:sx,height:sy,left:px,top:py});
					for(
let i = 0,leni = temp.sw.length,offx = 0;i<leni;i++){
						for(
let j = 0,lenj = temp.sh.length,offy = 0;j<lenj;j++){
...
let px = (base_width-sx)/2,py = (base_height-sy)/2;
					temp.o2.attr({width:sx,height:sy,left:px,top:py});
					for(
let i = 0,offx = 0;i<temp.sw.length;i++){
						for(
let j=0,lenj = temp.sh.length,offy = 0;j<lenj;j++){
...
return -1;
				...
				if(!system.multi.mode) fun_playback(\"@p\",system.txt.name);
...
let video = document.createElement(\"video\");
					document.querySelector(\"#sys_video\").append(video);
					video.src = source_url + sets.res.toLowerCase();
					video.width = system.ui.width; video.height = system.ui.height;
					video.autoplay = true;
...
/* before */
	if (system.txt.index >= system.txt.max){
		...
		fun_setting(\"reset\");
		document.getElementById(\"dialog_output\").innerHTML = \"鍓ф儏妯℃嫙宸茬粨鏉燂紝鍗曞嚮灏嗛噸鏂板紑濮嬪墽鎯呭洖椤綷";
		return;
	}
/* check */
...
let r,fun = \"\";
	...
	fun_msg(1,false,\"text data use [\"+ fun +\"] analyze complete.
return code:[\"+ r +\"]\");
}
...
function fun_fullscreen_support() {
	...
	/* if(!system.debug && (system.user.client == \"mobile\" || system.user.display == \"vert\")) 
return false; */
...
/**
 * 鍚戞帶鍒跺彴杈撳嚭涓€鏉″甫鏈夋椂闂翠俊鎭殑鏁版嵁 
 * @function 鍚戞帶鍒跺彴杈撳嚭淇℃伅
 * @description 鍚戞帶鍒跺彴杈撳嚭涓€鏉″甫鏈夋椂闂翠俊鎭殑鏁版嵁
 * @param msgs {String[]} 闇€瑕佽緭鍑虹殑娑堟伅
 * @param msg_type {Number} 娑堟伅绫诲瀷:Info=0,Debug=1,Warn=-1,Error=-2;Special color Info=2+
 * @param msg_debug {Boolean} 鏄惁鍙湪debug鐘舵€佷笅杈撳嚭娑堟伅:true(榛樿)=浠呭湪debug鐘舵€佷笅杈撳嚭娑堟伅,false=鏃犺浣曟椂閮戒細杈撳嚭娑堟伅
 * @
return void
 * @author Krliov
*/
...
function fun_playback(tar,args) {
...
let group = document.createElement(\"li\"), log;
	
let pas = (typeof args === \"object\") ? args : { name: args };
	
let name = pas.name, text = pas.text || system.txt.now;
...
let refs = sets.references ? sets.references.split(';') : [];
						if (logs.stacks.empty()) {
							support.log(LogType.warn, false, \"<Predicate>The stacks is empty.Has skipped the command.\");
							continue;
						}
						
let ptr = logs.stacks.last(), selv;
						do {
							
let refv = refs.filter((x) => ptr.values.includes(x));
...
let remv = ptr.values.filter((x) => !ptr.selected.has(x));
							...
							selv = refv.filter((x) => !ptr.selected.has(x));
							
break;

						} while (!logs.stacks.empty());
						if (logs.stacks.empty()) {
...
let op = logs.options[i];
							if (!op) {
								support.log(LogType.warn, false, `<Predicate>Can't find the options data of value [${op}].`);
								continue;
							}
							
let span = document.createElement('span');
							span.innerHTML = \"銆怽" + op + \"銆慭";
							ptr.selected.add(i);
							group.append(span);
						}
					}
