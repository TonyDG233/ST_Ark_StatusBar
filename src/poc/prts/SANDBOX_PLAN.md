...
### Phase 5: 视觉资产 (UI/CSS) 的分级存储与分发策略
针对一万多张高达数GB的角色立绘与场景大图，以及数十个核心UI图标，采用以下“分级白嫖”策略：
1. **核心 UI 图标 (UI Assets) -> 走独立 CDN 仓库分发**
   - 提取 `static.prts.wiki` 下报 403 拦截的核心图标（如播放、全屏等）。
   - 将这几十KB的素材存入独立的 GitHub 仓库，通过 jsDelivr 等无防盗链的 CDN 引入代码，确保界面永远不破相且不增加 JS 体积。
2. **角色立绘与背景大图 (Dynamic Assets) -> 实时直链 PRTS**
   - `media.prts.wiki` 域名下的大图无严格防盗链。
   - 在 Parser 和 Vue 组件中动态拼接 URL，利用玩家浏览器实时请求 PRTS 原始服务器，实现免存储、免带宽消耗。
