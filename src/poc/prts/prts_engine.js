			...
			case 'video':
				{
					...
					video.src = system.sourceUrl + sets.res.toLowerCase();
...
function fun_get_audio_url(key)
{
	...
	return key.startsWith("$") ? data.audio[p.substr(1)] : key.startsWith("@") ? system.assetUrl + p.substr(1) : system.assetUrl + p.replace("sound_beta_2", "audio") + ".mp3";
}
/**
 * 向控制台输出一条带有时间信息的数据 
 * @function 向控制台输出信息
