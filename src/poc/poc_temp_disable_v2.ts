...
async function testTempDisable() {
  ...
  eventOnce(tavern_events.GENERATION_ENDED, async message_id => {
    console.log(
      `%c[Temp Disable POC] 捕获到 GENERATION_ENDED (message_id: ${message_id})`,
    ...
    try {
      ...
      console.log(
        `%c[Temp Disable POC] 恢复完成! 请检查世界书面板是否重新开启，以及酒馆聊天是否有报错。`,
...
$(() => {
  // 创建触发按钮
  const btn = $(
    '<button style="position:fixed;top:60px;left:10px;z-index:9999;padding:10px;background:orange;color:black;">测试临时禁用</button>',
  );
