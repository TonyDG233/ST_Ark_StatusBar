...
function checkTimeTriggers(chronicle, globalTime) {
    ...
    if (chronicle.daily_summary_buffer.length >= 7) {
        return { taskType: 'weekly_summary', payload: { source_ids: chronicle.daily_summary_buffer.slice(0, 7).map(s => s.id) } };
    }
    ...
    if (chronicle.round_buffer.length > 0) {
        ...
        if (lastRoundDate && (globalDate.year > lastRoundDate.year || globalDate.month > lastRoundDate.month || globalDate.day > lastRoundDate.day) ) {
...
function scheduleTasks() {
    ...
    if (timeTrigger.taskType) {
        ...
        switch(timeTrigger.taskType) {
            case 'yearly_summary': priority = 40; break;
            case 'monthly_summary': priority = 30; break;
            case 'weekly_summary': priority = 25; break;
            case 'daily_summary': priority = 20; break;
        }
