<template>
  <div class="p-2 flex flex-wrap justify-between items-center bg-surface hover:bg-surface-container-highest transition-colors cursor-pointer border-b border-outline-variant group w-full box-border gap-2 relative"
    ...
    <div class="flex flex-wrap items-center gap-2 min-w-[120px] flex-1">
      ...
      <!-- Bind Type Badge -->
      <span class="font-display text-[10px] px-1.5 py-0.5 rounded-sm flex-shrink-0 font-bold whitespace-nowrap"
      ...
      <!-- Folder Title -->
      <h2 class="font-display text-[13px] font-bold transition-colors break-words whitespace-normal leading-tight"
          :class="expanded ? 'text-on-surface' : 'text-on-surface-variant group-hover:text-on-surface'">
        {{ title }}
      </h2>
      
      <!-- Count Badge -->
      <span class="bg-surface-container-highest border border-outline-variant text-on-surface-variant font-mono text-[9px] px-1.5 py-0.5 flex-shrink-0 whitespace-nowrap">
        {{ count }} 条
      </span>
    </div>
    ...
    <div class="flex items-center gap-2 flex-shrink-0">
      ...
      <button v-if="bindType !== 'char'" class="text-on-surface-variant transition-colors flex items-center justify-center outline-none w-6 h-6 rounded hover:bg-surface-variant cursor-pointer" 
        ...
        <span class="material-symbols-outlined text-[16px]">{{ bindType === 'global' ? 'link_off' : 'link' }}</span>
      </button>
      ...
      <!-- Expand Arrow -->
      <span class="material-symbols-outlined text-on-surface-variant transition-transform duration-200 text-[20px] flex-shrink-0 ml-1"
