<template>
  <div class="relative flex flex-col mb-6">
    ...
    <div
      class="absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 bg-background box-border"
      :class="isSnapshot ? 'border-secondary' : 'border-primary'"
    ></div>

    <div
      class="flex flex-col border border-outline-variant bg-surface-container-lowest p-3 group transition-colors min-w-0"
      :class="[
        isSnapshot ? 'hover:border-secondary/50' : 'hover:border-primary/50',
        isBatchMode ? 'cursor-pointer hover:bg-surface-variant' : '',
      ]"
    ...
    >
      ...
      <div class="flex justify-between items-start gap-2 mb-1">
        ...
        <label v-if="isBatchMode" class="shrink-0 pt-0.5 cursor-pointer flex items-center group" @click.stop>
          ...
          >
            <span v-if="isSelected" class="material-symbols-outlined text-[calc(12em/14)] text-on-primary font-bold"
              >check</span
            >
          </div>
        </label>
        ...
        <div class="flex flex-col min-w-0 flex-1">
          <div
            class="text-[calc(10em/14)] font-mono text-on-surface-variant opacity-80 flex items-center gap-2 flex-wrap"
          >
          ...
          <!-- Title -->
          <div
            class="text-sm font-display font-bold text-on-surface tracking-wide mt-1 break-words whitespace-normal leading-tight"
          >
            {{ title }}
          </div>
        </div>
      ...
      <div
        v-if="description"
        class="text-[calc(11em/14)] text-on-surface-variant leading-relaxed mb-3 break-words min-w-0"
      >
        {{ description }}
      </div>
      ...
      <div
        ...
        class="flex flex-col gap-1.5 bg-surface-variant/20 p-2 border border-outline-variant/30 mb-3 rounded-sm min-w-0"
      >
        <div
          v-for="(change, idx) in changes"
          :key="idx"
          class="text-[calc(10em/14)] text-on-surface break-words whitespace-normal leading-tight"
        >
