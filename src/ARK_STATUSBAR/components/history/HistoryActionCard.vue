<template>
  <div
    class="border p-3 flex flex-col relative bg-surface-container-low transition-colors box-border"
    :class="[
  ...
  >
    ...
    <div class="flex flex-col relative group">
      <div
        class="text-[calc(10em/14)] font-display font-bold tracking-wider uppercase mb-1 whitespace-normal"
        :class="type === 'danger' ? 'text-error' : type === 'primary' ? 'text-primary-text' : 'text-secondary'"
      >
        {{ label }}
      </div>
      <div
        class="text-sm font-display font-bold text-on-surface tracking-wide whitespace-normal"
        :class="type === 'danger' ? 'text-error' : ''"
      >
        {{ title }}
      </div>
      <div
        class="text-[calc(11em/14)] mt-1 leading-tight break-words whitespace-normal min-w-0 pr-6"
      ...
      <span
        class="material-symbols-outlined absolute top-0 right-0 text-lg opacity-80 group-hover:opacity-100 transition-opacity"
        :class="type === 'danger' ? 'text-error' : type === 'primary' ? 'text-primary-text' : 'text-secondary'"
      >
        {{ icon }}
      </span>
