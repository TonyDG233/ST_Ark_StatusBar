<template>
  <div
    class="ark-mini-window w-full bg-surface border-x border-b border-outline-variant flex flex-col relative overflow-hidden"
  ...
  >
    ...
    <ul v-else class="flex-1 overflow-y-auto scrollbar-none flex flex-col gap-1 min-h-0 px-3 py-2">
      <li
        ...
        :key="index"
        class="flex items-center gap-2 py-1 border-b border-outline-variant/30 last:border-0"
      >
        <div
          class="w-1.5 h-1.5 flex-shrink-0 bg-primary"
          :class="{ '!bg-error': entry.enabled === false && !entry.tempDisabled }"
        ></div>
        <span
          class="text-[calc(11em/14)] font-body text-on-surface truncate flex-1 min-w-0"
          :class="{ 'opacity-50 line-through': entry.enabled === false && !entry.tempDisabled }"
          >{{ entry.name }}</span
        >
      </li>
    </ul>
