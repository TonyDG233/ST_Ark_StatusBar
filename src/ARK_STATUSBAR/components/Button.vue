<template>
  <button
    ...
    :class="[
      ...
      variant === 'outline'
        ? 'bg-transparent text-primary-text border-outline hover:border-primary hover:bg-primary/10'
        : '',
      variant === 'ghost'
        ? 'bg-transparent text-on-surface-variant border-transparent hover:text-on-surface hover:bg-surface-variant'
        : '',
  ...
  >
    ...
    <div
      v-if="variant !== 'ghost'"
      class="absolute bottom-0 right-0 w-2 h-2 bg-background transform translate-x-1 translate-y-1 rotate-45 border-l border-t"
      :class="variant === 'primary' ? 'border-primary' : 'border-outline group-hover:border-primary'"
    ></div>
  </button>
</template>
