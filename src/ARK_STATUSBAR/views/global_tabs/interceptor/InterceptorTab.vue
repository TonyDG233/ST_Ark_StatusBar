<template>
  <div class="relative w-full h-full overflow-hidden flex flex-col box-border">
    ...
    <div class="p-2 flex flex-col gap-2 box-border flex-1 min-h-0 overflow-y-auto ark-scrollbar pb-14">
      ...
      >
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 mt-1 w-full">
          ...
          >
            ...
            >
            <div
              class="relative w-8 h-4 flex items-center p-0.5 transition-colors border border-outline/50"
              :class="
                currentConfig?.isInterceptorEnabled
                  ? 'bg-primary justify-end border-primary'
            ...
            >
              <input
                type="checkbox"
                class="hidden"
                :checked="currentConfig?.isInterceptorEnabled"
      ...
      <div
        v-if="!currentConfig?.isInterceptorEnabled && !isTestMode"
        class="text-on-surface-variant text-sm p-4 text-center border border-dashed border-outline-variant/50 mt-4 flex flex-col items-center gap-2"
      >
        <span class="material-symbols-outlined text-outline">warning</span>
        预检拦截已关闭，发送请求将直接通行。
      </div>
      <div
        v-else-if="pendingEntries.length === 0"
        class="text-on-surface-variant text-sm p-4 text-center border border-dashed border-outline-variant/50 mt-4"
      >
        ...
        <div v-else class="flex flex-col items-center">
          ...
          <button
            class="mt-4 bg-primary/10 text-primary-text border border-primary/30 px-3 py-1.5 font-bold text-[calc(13em/14)] hover:bg-primary/20 transition-colors break-words whitespace-normal text-center"
      ...
      <template v-else>
        ...
        <div class="flex flex-col gap-2 w-full mt-1">
          <div
            class="font-display text-xs font-bold tracking-widest uppercase text-on-surface-variant border-b border-surface-variant pb-1 flex flex-wrap justify-between items-center gap-1"
          >
            <span>待处理队列 (PENDING ENTRIES)</span>
            <button
              v-if="isTestMode"
              class="text-[calc(11em/14)] text-primary hover:underline whitespace-normal break-words text-right"
              @click="clearTestResults"
            >
              清除测试结果
            </button>
        ...
        <div class="w-full mt-4 flex flex-wrap gap-2" v-if="!isTestMode">
          <button
            class="bg-surface border border-error text-error font-display text-[calc(11em/14)] font-bold tracking-widest uppercase py-2.5 hover:bg-error hover:text-on-error transition-colors flex items-center justify-center gap-1 flex-1 min-w-[120px] outline-none"
            @click="cancelSend"
          >
          ...
          <button
            class="bg-primary text-on-primary font-display text-[calc(11em/14)] font-bold tracking-widest uppercase py-2.5 hover:bg-primary-container transition-colors flex items-center justify-center gap-1 flex-1 min-w-[120px] outline-none shadow-[0_0_8px_rgba(78,213,255,0.4)]"
            @click="confirmSend"
          >
    ...
    >
      ...
      >
        <!-- Header -->
        <div
          class="flex justify-between items-center border-b border-outline-variant bg-surface/50 p-2 gap-2 flex-shrink-0"
        >
          <div class="flex flex-col min-w-0">
            <div class="flex items-center gap-1.5">
              <!-- Type Badge -->
              <span
                v-if="selectedEntry.strategy?.type === 'constant'"
                class="text-[10px] bg-[#4ed5ff]/20 text-[#4ed5ff] border border-[#4ed5ff]/30 px-1 py-0.5 rounded-sm font-bold uppercase whitespace-nowrap"
              >
                🔵 常驻
              </span>
              <span
                v-else
                class="text-[10px] bg-[#afd439]/20 text-[#afd439] border border-[#afd439]/30 px-1 py-0.5 rounded-sm font-bold uppercase whitespace-nowrap"
              >
                🟢 条件
              </span>
              <!-- Title -->
              <h3
                class="font-display font-bold text-on-surface text-[calc(13em/14)] truncate uppercase"
        ...
        <div class="p-3 flex flex-col gap-3 overflow-y-auto ark-scrollbar min-h-0 flex-1">
          ...
          <div v-if="selectedEntry.strategy?.type === 'selective'" class="flex flex-col gap-1 w-full min-w-0">
            ...
            <div
              class="font-mono text-[calc(11em/14)] text-on-surface-variant bg-surface border border-outline-variant/50 p-1.5 break-words whitespace-normal w-full min-w-0"
            >
          ...
          <div class="flex flex-col gap-1 flex-1 min-h-0">
            <div class="flex flex-wrap justify-between items-end gap-1 w-full">
              ...
              <span
                class="font-mono text-[9px] text-on-surface-variant bg-surface-variant/50 border border-outline-variant/30 px-1 py-0.5 rounded-sm whitespace-nowrap"
              >
                ~{{ entryTokenCountCache[getEntryKey(selectedEntry)] || 0 }} TOKENS
              </span>
            </div>
            <div
              class="font-mono text-[calc(11em/14)] leading-relaxed text-on-surface bg-surface border border-outline p-2 whitespace-pre-wrap break-words overflow-y-auto ark-scrollbar flex-1"
            >
...
const { currentPrimaryWorldbook, currentTokenCount, entryTokenCountCache, isTestMode, pendingEntries } =
  storeToRefs(uiStore);

const { getEntryKey } = uiStore;
