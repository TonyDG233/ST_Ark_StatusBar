  ...
  <div class="flex flex-col bg-surface-container-lowest border-x border-b border-outline-variant w-full box-border">
    ...
    <div v-if="isEntryBatchMode" class="bg-surface-container-highest border-b border-outline-variant py-1.5 px-2 flex flex-col sm:flex-row flex-wrap sm:items-center justify-between gap-2 shadow-sm">
      ...
      <div class="flex flex-wrap items-center gap-1.5">
        <!-- Pin/Unpin -->
        <button class="px-1.5 py-0.5 border border-outline bg-surface hover:bg-surface-variant text-on-surface text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="actions.batchPinEntries(selectedEntries, true)">
          <span class="material-symbols-outlined text-[calc(12em/14)] text-primary-text">push_pin</span> 置顶
        </button>
        <button class="px-1.5 py-0.5 border border-outline bg-surface hover:bg-surface-variant text-on-surface text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="actions.batchPinEntries(selectedEntries, false)">
          <span class="material-symbols-outlined text-[calc(12em/14)] text-on-surface-variant">push_pin</span> 消顶
        </button>
        <!-- Blue/Green Toggle -->
        <button class="px-1.5 py-0.5 border border-outline bg-surface hover:bg-surface-variant text-on-surface text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="actions.batchToggleEntryType(wbName, selectedEntries)">
          <span class="material-symbols-outlined text-[calc(12em/14)] text-secondary">change_circle</span> 类型切换
        </button>
        <!-- Enable/Disable -->
        <button class="px-1.5 py-0.5 border border-outline bg-surface hover:bg-surface-variant text-secondary text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="actions.batchToggleEntryEnabled(wbName, selectedEntries, true)">
          <span class="material-symbols-outlined text-[calc(12em/14)]">check_circle</span> 开启
        </button>
        <button class="px-1.5 py-0.5 border border-outline bg-surface hover:bg-surface-variant text-on-surface-variant text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="actions.batchToggleEntryEnabled(wbName, selectedEntries, false)">
          <span class="material-symbols-outlined text-[calc(12em/14)]">block</span> 关闭
        </button>
        <!-- Delete -->
        <button class="px-1.5 py-0.5 border border-error/50 bg-error-container/10 hover:bg-error-container/30 text-error text-[calc(10em/14)] flex items-center gap-0.5 transition-colors outline-none cursor-pointer" @click="handleBatchDelete">
