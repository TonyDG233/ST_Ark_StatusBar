<template>
  ...
  <template v-if="store.runtimes.unsupported_warnings !== '' && store.settings.更新方式 === '额外模型解析'">
    <div class="mvu-warning">
      <span class="mvu-warning__icon">ℹ️</span>
      <span class="mvu-warning__text">
        世界书 [{{ store.runtimes.unsupported_warnings }}] 未适配额外模型解析, 视为 [mvu_plot] 条目 (只会发给剧情
        AI、不会发给变量更新 AI).
...
import update_method_help from '../update_method.md';
