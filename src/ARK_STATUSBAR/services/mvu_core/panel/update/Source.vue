<template>
  <Detail title="模型来源">
    <Select v-model="store.settings.额外模型解析配置.模型来源" :options="['与插头相同', '自定义']" />
    ...
    <template v-if="store.settings.额外模型解析配置.模型来源 === '自定义'">
      ...
      <Detail title="高级参数">
        <div v-if="!additional_extra_configuration_supported" class="mvu-note">⚠️酒馆助手版本过低，不支持以下配置</div>
...
import ModelSelect from '../component/ModelSelect.vue';
import RangeNumber from '../component/RangeNumber.vue';
