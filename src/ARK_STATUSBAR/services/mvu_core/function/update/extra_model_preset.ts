...
function toOrderedPrompt(prompt: ExtraModelPresetPrompt): GenerateRawOrderedPrompt | null {
  const builtin_prompt =
    preset_placeholder_to_builtin_prompt[prompt.id as keyof typeof preset_placeholder_to_builtin_prompt];
  if (builtin_prompt) {
...
export function buildOtherPresetGenerateConfig(
  ...
  const injects: GenerateInjectPrompt[] = [...getDefaultTaskInjects(task), ...preset_in_chat_injects];
  ...
  return {
    ...
    request_overrides: {
      ...
      ...(isFiniteNumber(settings.frequency_penalty) ? { frequency_penalty: settings.frequency_penalty } : {}),
      ...(isFiniteNumber(settings.presence_penalty) ? { presence_penalty: settings.presence_penalty } : {}),
      ...(isFiniteNumber(settings.repetition_penalty) ? { repetition_penalty: settings.repetition_penalty } : {}),
