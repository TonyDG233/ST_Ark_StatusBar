...
import { cleanUpMetadata, generateSchema, getSchemaForPath, reconcileAndApplySchema } from './schema';
...
export function extractCommands(inputText: string): Command[] {
  ...
  const results: (Command & { $index: number })[] = _.concat(
    [...inputText.matchAll(/<(json_?patch)>(?:\s*```.*)?((?:(?!<json_?patch>)[\s\S])*?)(?:```\s*)?<\/\1>/gim)]
      .map(match => ({
        index: match.index ?? 0,
        string: match[2].trim(),
      }))
  ...
  while (i < inputText.length) {
    ...
    else if (commandType === 'add' && /*params.length === 1 || */ params.length === 2) isValid = true; // _.add 需要1个或2个参数
...
export async function updateVariable(
  ...
  if (_.has(stat_data, path)) {
    ...
    if (Array.isArray(currentValue) && currentValue.length === 2) {
      ...
      if (is_recursive) await eventEmit(variable_events.SINGLE_VARIABLE_UPDATED, stat_data, path, oldValue, new_value);
      return true;
    } else {
...
export async function updateVariables(current_message_content: string, variables: MvuData): Promise<boolean> {
  ...
  await eventEmit(variable_events.COMMAND_PARSED + '_for_zod', variables, commands, current_message_content);
  //允许 MVU zod 在处理完所有 COMMAND_PARSED 后清理 commands
  await eventEmit(variable_events.COMMAND_PARSED + '_ended_for_zod', variables, commands, current_message_content);
  ...
  for (const command of commands) {
    ...
    switch (
      ...
      case 'set': {
        ...
        let oldValue = path === '' ? klona(variables.stat_data) : _.get(variables.stat_data, path);
        ...
        // 触发单变量更新事件，通知外部系统
        await eventEmit(variable_events.SINGLE_VARIABLE_UPDATED, variables.stat_data, path, oldValue, finalNewValue);
        break;
      }
      ...
      case 'assign': {
        ...
        if (successful) {
          // 插入成功，获取新值并触发事件
          const newValue = isNullOrWhiteSpace(path) ? variables.stat_data : _.get(variables.stat_data, path);
          console.info(display_str);
          await eventEmit(variable_events.SINGLE_VARIABLE_UPDATED, variables.stat_data, path, oldValue, newValue);
          try {
      ...
      case 'remove': {
        ...
        if (targetToRemove === undefined) {
          ...
          itemRemoved = true;
          await eventEmit(variable_events.SINGLE_VARIABLE_UPDATED, variables.stat_data, path, oldValue, undefined);
        } else {
...
export async function handleVariablesInMessage(message_id: number) {
  ...
  if (chat_message.role !== 'user') {
    ...
    if (message_content.includes('<status_current_variable>')) {
      message_content = message_content.replaceAll(/<(status_current_variable)>(?:(?!<\1>).)*<\/\1?>/gis, '');
    }
