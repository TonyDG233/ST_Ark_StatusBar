import { correctlyMerge, parseString } from '@/src/util/common';
...
export async function initCheck() {
  ...
  if (is_updated || !variables.schema || _.isEmpty(variables.schema)) {
    ...
    const generated_schema: SchemaNode & RootAdditionalProps = generateSchema(dataForSchema, variables.schema);

    // 使用类型守卫确保生成的 schema 是 ObjectSchemaNode
    if (isObjectSchema(generated_schema)) {
      if (_.has(variables.stat_data, '$meta.strictTemplate'))
        generated_schema.strictTemplate = variables.stat_data['$meta']?.strictTemplate as boolean;
      if (_.has(variables.stat_data, '$meta.concatTemplateArray'))
        generated_schema.concatTemplateArray = variables.stat_data['$meta']?.concatTemplateArray as boolean;
    ...
    await setChatMessages([
      {
        ...
        swipes_data: await Promise.all(
          last_msg.swipes!.map(async (swipe, index) => {
            let vanilla_variable_data: Record<string, any> = klona(last_msg.swipes_data[index]);
            ...
            const matched_init = swipe.matchAll(/<(initvar)>(?:\s*```.*)?([\s\S]*?)(?:```\s*)?<\/\1>/gim);
            ...
            if (is_initvar_applied) {
              ...
              const char_lorebook = getCharWorldbookNames('current').primary ?? 'unknown';
  ...
  try {
    if (useDataStore().settings.通知.变量初始化成功) {
      // 输出构建信息
      toastr.info(
        `有新的世界书初始化变量被加载，当前使用世界书:<br>${Object.entries(variables.initialized_lorebooks ?? {})
 ...
 */
export async function loadInitVarData(mvu_data: MvuData, lorebook_list?: string[]): Promise<boolean> {
