...
export async function restoreVariables() {
  ...
  const last_20th_message_id = Math.max(1, last_message_id - store.settings.自动清理变量.要保留变量的最近楼层数);
  ...
  let variables = klona(snapshot_chat_message.variables![snapshot_chat_message.swipe_id ?? 0] as MvuData);
