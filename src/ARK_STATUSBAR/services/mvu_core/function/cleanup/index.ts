...
export function initCleanup() {
  ...
  stop_list.push(controlledStoppableEventOn(tavern_events.MESSAGE_DELETED, _.debounce(restoreVariables, 2000)));
  stop_list.push(
    controlledStoppableEventOn(tavern_events.MESSAGE_RECEIVED, message_id => {
      ...
      if (old_message_id > 0) {
        const counter = cleanupMessageVariables(
          ...
          Math.max(1, old_message_id - 2 - store.settings.自动清理变量.要保留变量的最近楼层数 * 2), // 因为没有监听 MESSAGE_SENT
          old_message_id,
