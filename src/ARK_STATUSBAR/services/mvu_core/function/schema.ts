...
export function generateSchema(
  ...
  if (Array.isArray(data)) {
    ...
    const metaElementIndex = data.findIndex(
      item =>
        _.isObject(item) && !_.isDate(item) && '$arrayMeta' in item && '$meta' in item && item['$arrayMeta'] === true,
    );
    ...
    const schema_node: ArraySchemaNode = {
      ...
      elementType: data.length > 0 ? generateSchema(data[0], oldElementType, isRecursiveExtensible) : { type: 'any' },
    };
 ...
 */
export function getSchemaForPath(schema: SchemaNode | null | undefined, path: string): SchemaNode | null {
  if (!path || !schema) {
    return schema || null;
  }
