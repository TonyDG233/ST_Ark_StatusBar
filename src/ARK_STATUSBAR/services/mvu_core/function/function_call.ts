...
export function overrideToolRequest(generate_data: any) {
  const store = useDataStore();
  if (store.settings.更新方式 !== '额外模型解析' || store.settings.额外模型解析配置.应答格式 !== '工具调用') {
    return;
  }
  ...
  try {
    ...
    if (json.delta && json.delta.length > 5) {
      ...
      try {
        ...
        if (json_patch_match) {
          console.error(`[MVU额外模型解析]无法解析的变量更新块。 ${json.delta}, 错误 ${error}`);
          return null;
        }
        const fn_call_match = /_\.(?:set|insert|assign|remove|unset|delete|add)\s*\([\s\S]*?\)\s*;/.test(json.delta);
...
export function extractFromFormattedOutput(result: string | GenerateToolCallResult): string | null {
  ...
  try {
    ...
    const patch_source = isJsonPatch(parsed)
      ? parsed
      : (_.get(parsed, 'json_patch') ?? _.get(parsed, 'jsonPatch') ?? _.get(parsed, 'patch') ?? _.get(parsed, 'delta'));
    const json_patch = normalizeJsonPatchPayload(patch_source);
    if (!json_patch) {
      throw new Error('不是有效的 json patch');
    }
    const analysis = isJsonPatch(parsed) ? '' : (_.get(parsed, 'analysis') ?? _.get(parsed, 'analyze') ?? '');
