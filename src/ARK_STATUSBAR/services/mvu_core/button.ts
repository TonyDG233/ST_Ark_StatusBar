...
export const buttons: Button[] = [
  ...
  },
  {
    ...
    function: async () => {
      ...
      SillyTavern.saveChat().then(() => toastr.success(`已将 ${message_id} 层配置为快照楼层`, '[MVU]配置楼层快照'));
    },
  },
  {
    name: '重演楼层',
    ...
    function: async () => {
      ...
      for (let i = recur_intial_message_id + 1; i <= message_id; i++) {
        ...
        counter++;
        if (counter % 50 === 0) {
          toastr.info(`处理变量中 (${counter} / ${message_id - recur_intial_message_id})`, `[MVU]楼层重演`);
        }
      }
      ...
      SillyTavern.saveChat().then(() =>
        toastr.success(`已将 ${message_id} 层变量状态重演完毕，共重演 ${counter} 楼`, '[MVU]楼层重演'),
      );
  ...
  },
  {
    ...
    function: async () => {
      ...
      if (store.settings.更新方式 === '随AI输出') {
        ...
        return;
      } else if (store.settings.额外模型解析配置.应答格式 === '工具调用' && !isFunctionCallingSupported()) {
        toastr.info(
      ...
      if (begin_pos >= 0) {
        ...
        if (end_pos === -1) {
          ...
          filtered_string = current_chat_content.slice(0, begin_pos) + current_chat_content.slice(end_pos + 17);
        }
        //更新聊天记录
        await setChatMessages(
          [
    ...
    function: async () => {
      ...
      SillyTavern.chat.slice(1, -depth - 1).forEach((chat_message, index) => {
        ...
        chat_message.variables = _.range(0, chat_message.swipes?.length ?? 1).map(i => {
          ...
          if (_.get(chat_message.variables[i], 'snapshot') === true) return chat_message.variables[i];
...
export function initButtons() {
  ...
  const stop = watch(
    ...
    should_enable => {
      ...
      const existing_buttons = _.intersectionBy(current_buttons, buttons, button => button.name);
