...
## 3. 系统架构状态字典
📂 **`.kilocode/state/`**
- 本目录包含项目的绝对当前状态，由 `PRD.md` (需求)、`ARCH.md` (架构结论与防线规约) 和 `PROJECT_STATE.md` (当前进度看板) 组成。
- **每次开启新任务或新会话前必须优先阅读此目录下的文件**，以对齐项目当前的绝对真相。
...
## 6. PoC 概念验证脚本
📂 **`src/poc/`**
- 此目录已经存在，包含为了验证某个特性（如酒馆原生事件、Token 计算等）是否能在酒馆环境中执行而编写的独立前端脚本。
- 新的 PoC (Proof of Concept) 脚本应统一存放在该目录下，请务必作为 `.js` 或 `.ts` 文件单独提交，以便在浏览器控制台中直接运行测试。

## 7 项目架构与技术选型规约
* **TECH_STACK.md** (位于 .kilocode/rules/TECH_STACK.md)
* 记载了事件总线（Event System）、数据流等底层的必须遵从的实操规范，请在涉及跨模块通信或架构重构时务必查阅。
