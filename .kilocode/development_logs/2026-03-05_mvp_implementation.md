...
## 2026-03-06: 恶性 Bug 修复 (18 vs 36 条目丢失问题)
- **问题描述**：在执行“主动检测”时，系统只检测到了 18 个条目（多为常驻或角色绑定的蓝灯条目），而真实发送时却能触发 36 个条目。所有依赖于聊天历史记录中关键词（例如“维多利亚”触发“多利”）的动态条目全部丢失。
- **排查与对象分析过程**：
  - 尝试直接传递深拷贝的 ST 原生 `chat` 消息对象数组，但导致 ST 引擎崩溃并抛出 `.trim is not a function` 错误，因为 ST 底层强依赖纯字符串。
  - 直接越界深入研究酒馆宿主的原生代码：`SillyTavern/public/scripts/world-info.js`。通过查阅 `checkWorldInfo` 等五千多行的逻辑，发现了 ST 引擎对上下文扫描的两个致命隐藏规则：
    1. **消息格式**：传入的数组必须是纯字符串，并且严格按照 `Name: Message` 格式拼接（若缺少角色名，ST 可能会无法正确识别深度，或在其他正则处理时失效）。
    2. **逆序排列 (Critical)**：ST 的深度（Depth/Search Range）计算逻辑**认定索引 `0` 为最新消息**。之前我们传入的是正序数组，导致 `Depth: 1` 永远只扫描对话最开头的远古记录，从而完美错过了当前上下文中包含的所有触发关键词。
- **最终修复方案**：
  - 在 `statusbar_manager.ts` 中构建 `mockChat` 时，主动判断 `msg.is_user` 并拼接 `Name: Message`。
  - 在传递给 `getWorldInfoPrompt` 之前，执行 `mockChat.reverse()` 将正序数组翻转为倒序。修复后，主动检测能成功精准命中与原生一致的 36 个世界书条目。
