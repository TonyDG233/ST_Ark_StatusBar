...
*   **难点**：现存大量由纯文本或 YAML 编写的“幕 (Acts)”与“节点 (Nodes)”剧情，如何将它们转换为本插件能理解的结构？
*   **PoC 方向**：
    1.  **解析器编写**：设计一套读取规则，将 YAML/文本按特定标识（如 `剧情节点RI1`）拆分为一个个包含 `NodeID`、`Summary`（给小模型看）和 `FullScript`（给大模型看）的内存对象。（注意如果社区撰写的不同剧情世界书格式有所区别，需要定义一个稳定的剧情模板来供社区的大伙/agent进行归一化操作，避免读取失败。同时剧情逻辑也得尝试转化为mermaid或者其他能稳定表示逻辑关系的方法给次级api/插件UI读取，用于提示词或者显示给用户看。）
    2.  **次级 API 思维链的归宿**：明确思路是作为世界书条目存在，还是挂载在聊天变量中，或是直接隐身在消息文本中？
    3.  **兼容“手动模式”**：必须保留兼容当前社区手动切换的模式（即不拦截发送，仅提供 UI 辅助查询节点与思路生成功能，通过往用户消息中注入思维链实现原先效果/提供完整提示信息引导用户修改预设实现原有功能）。
