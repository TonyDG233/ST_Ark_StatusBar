...
## 二、 新增核心实现：无缝变身气泡窗 (Bubble Mode)

在任务推进过程中，针对原沙盒版中生硬独立的 `<div class="edge-snap-indicator">`，提出并实现了一套更为优雅的“无缝变身”方案，现作为架构资产补充记录：

### 核心设计：本体极限压缩与动态圆角欺骗

放弃创建新的气泡 DOM，而是让原本的主面板 (`.ark-visual-panel`) 直接进入极限压缩态 (`UiMode.BUBBLE`)。

1.  **高度与宽度接管**：进入气泡模式时，面板高度被强制压扁（如 60px）。其 CSS `--ui-width` 变量则被移交给物理引擎的 `snappedStretchWidth`（默认 32px，向内拖拽时最大 80px 的橡皮筋宽度）接管。
2.  **内容替换**：在 Vue 模板中通过 `v-show="currentUiMode === UiMode.BUBBLE"`，隐藏原本的 Header 和 List，仅渲染一个居中的书本图标。
3.  **动态圆角（核心视觉魔术）**：原本面板四角皆有圆角（如 8px）。当面板被压扁并死锁在屏幕边缘时，通过传入的 `isSnappedToEdge` 状态赋予对应的类名（如 `.bubble-left`）：
    *   **左贴边 (`.bubble-left`)**：`border-radius: 0 30px 30px 0;`。靠墙面（左）的圆角被削平为直角 `0`，而朝内的面变成了完美的 `30px` 半圆弧。
    *   **右贴边 (`.bubble-right`)**：`border-radius: 30px 0 0 30px;`。
    
**最终效果**：由于面板本来就带有的毛玻璃背景 (`transparent-theme`) 和阴影没有改变，配合削平一半的圆角，在视觉上完美伪装成了一个从屏幕边缘自然“生长”出来的半圆形把手。且由于是复用同一个 DOM，这让面板从“气泡状态”瞬间释放成“完整列表状态”时的过度（如果开启 CSS 过渡）显得非常平滑和连贯。

---

## 三、 明天的工作步骤与护城河 (The Executable Steps)
