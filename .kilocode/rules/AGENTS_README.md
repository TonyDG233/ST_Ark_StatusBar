...
### 2.1 重建信任工作流 (Trust Rebuilding Workflow)
**背景**: 在经历了 Stage 3 初期的灾难性失败后，为确保 Agent 不再因错误的理解和疏忽而产出低质量工作，特此建立本工作流。**此流程拥有最高优先级，是所有复杂模块开发的标准作业程序 (SOP)。**

1.  **强制学习 (Mandatory Study Session)**:
    *   在开始任何**新类型**或**高复杂度**模块的规划前，Agent 必须首先识别出所有相关的**核心参考文档**（API文档、核心设计思想、既有实现等）。
    *   Agent 必须逐一阅读这些文档，并在内部思维链或规划文档的开头，明确列出从每份文档中提炼出的**核心知识点**。

2.  **清理与重启 (Clean Slate Protocol)**:
    *   如果一个规划方向被证明是错误的，必须立即停止。所有相关的失败设计文档都将被归档至 `backup` 目录，以确保后续工作从一个干净、无污染的状态开始。

3.  **总分结构规划 (Master-Detail Planning)**:
    *   **总纲 (Master Plan)**: 必须先创建一个顶层的总括规划文件。该文件只负责定义**“做什么” (What)**，包括：待办文件清单、各文件核心功能、开发顺序，以及每个功能点所对应的**原始用户反馈**。
    *   **详细设计 (Detail Design)**: 针对总纲中的每一个复杂组件，都必须创建一个独立的、极其详尽的设计文档。该文档负责定义**“怎么做” (How)**。

4.  **单点验证 (Single-Point Verification)**:
    *   在完成第一个详细设计文档后，必须**立即停止**所有其他规划工作，并将其提交给用户进行**审核**。
    *   这份文档将作为对 Agent 理解能力和执行能力的**“资格考试”**。
    *   **问题缓存区**: 设计文档中必须包含一个专门的“问题缓存区”或“迭代与反馈”章节。所有在审核过程中发现的新问题、不明确的需求或关联挑战，都将记录于此，确保问题不会被遗忘，并为后续的专项讨论提供依据。
    *   只有在这份“考卷”得到用户明确批准后，Agent 才能被授权继续进行下一个组件的详细设计。此过程循环往复，直到所有组件的设计都通过验证。

5.  **流程固化 (Process Cementing)**:
    *   在工作流被验证有效后，必须将其经验总结并补充到本 `AGENTS_README.md` 文件中，使其成为项目不可分割的一部分。

### 2.2 协同工作协议 (Collaborative Protocol)
...
### 2.3 新协作标准作业程序 (New Collaborative SOP)

**背景**: 在完成了 Phase 3 的核心开发后，我们沉淀并固化了一套高精度、高效率的协作流程。此流程将作为后续所有复杂功能开发和重构的指导原则。

**核心思想**: **规划驱动、逐级确认、绝不臆测**

1.  **宏观规划 (纵向 & 横向)**:
    *   **纵向规划**: 按功能模块（如 Player, Character, Chronicle）进行独立、深入的功能与数据结构设计。
    *   **横向规划**: 按业务逻辑（如 `变量更新规则`, `变量列表渲染`, `任务执行`）进行跨模块的整合与协同规划，确保数据流和逻辑流的全局一致性。
    *   **实施蓝图**: 必须在完成上述两个维度的规划后，将其整合成一份统一的、可执行的**实施蓝图**（如 `038_Phase3_EJS_Implementation_Blueprint.md`），方可进入编码阶段。

2.  **微观执行 (事前校验 & 分步确认)**:
    *   **任务拆分**: 严格按照“文件 → 文件内区块”的粒度，将实施蓝图拆分为极其细致的 `todo` 列表。
    *   **事前学习**: 在执行任何一个 `todo` 项之前，**必须**重新阅读所有与之相关的 Schema 定义、最终规划文档和已有的相关代码，以建立完整的上下文。
    *   **方案先行**: 在阅读理解后，**必须**以“伪代码”或“最终产出物”的形式，向用户清晰地陈述将要执行的具体操作和内容。
    *   **批准后执行**: **只有在获得用户的明确批准后**，才能切换到 `code` 模式进行文件写入或修改。此原则神圣不可侵犯。
    *   **不懂就问**: 在任何环节遇到任何疑问或不确定性，都**必须**立即停止，并通过提问来解决。臆测和假设是失败的根源。
